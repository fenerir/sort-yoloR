...
def detect(opt, *args):
    out, source, weights, view_img, save_txt, imgsz,cfg, save_img, sort_max_age, sort_min_hits, sort_iou_thresh= \
        opt.output, opt.source, opt.weights, opt.view_img,opt.cfg, opt.save_txt, opt.img_size, opt.save_img, opt.sort_max_age, opt.sort_min_hits, opt.sort_iou_thresh
    ...
    #model = torch.load(weights, map_location=device)['model'] #load to FP32. yolov5s.pt file is a dictionary, so we retrieve the model by indexing its key
